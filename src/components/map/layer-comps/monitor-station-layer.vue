...
export default {
  methods: {
    ...
    getIcon (feature, hover = false, context) {
      ...
      //     `
      //   rain.map(e => {
      //     _html += `<div class="${pointValueClass}">${e}</div>`
      //   })
      //   _html += '</div>'
      // } else {
      //   _html = `
      ...
      var divIconBig = L.divIcon({
        className: `monitor-station-layer${hover ? ' list-hover' : ''} monitor-station${feature.properties.stationcode}`
        // html: _html
      })
      return divIconBig
    },
